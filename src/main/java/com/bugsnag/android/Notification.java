...
public class Notification {
    ...
    public Notification(Configuration config) {
        ...
        try {
            ...
            Logger.warn("Unable to create notification stream", e);
        }
    }
    ...
    public void setError(Error error) {
        if(error != null) {
            try {
                ...
                Logger.warn("Unable to stream error to bugsnag", e);
            }
        }
    }
    ...
    public void setError(File file) {
        if(file != null && file.exists() && file.isFile()) {
            try {
                ...
                Logger.warn("Bugsnag error file not found, but file exists...", e);
            }
        }
    }
    ...
    public void deliver() throws NetworkException {
        if(errorStream != null && firstNotificationStream != null && secondNotificationStream != null) {
            ...
            Logger.info(String.format("Sent 1 error to Bugsnag (%s)", url));

            try { sis.close(); } catch (java.io.IOException e){Logger.warn("Unable to close stream in bugsnag", e);}
            try { errorStream.close(); } catch (java.io.IOException e){Logger.warn("Unable to close stream in bugsnag", e);}
            errorStream = null;
        }
    }
}
